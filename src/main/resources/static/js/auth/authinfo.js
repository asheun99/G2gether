...
function updateRole() {
	// 1. 입력값 가져오기
	const roleCode = document.getElementById('roleCode').textContent.trim();
	...
	rows.forEach(row => {
		...
		const raCode = row.querySelector('.raCode').textContent.trim();
		permissions.push({
			category: category,
			rdRol: rdRol,
			wrRol: wrRol,
